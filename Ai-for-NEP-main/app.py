...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        "Choose AI Model",
        options=["dolly_chatbot_model", "gpt-neo-125M"],
        index=0
    )
    ...
    **Nexo** is an AI assistant created by **Aayush Bohora**. 
    It's powered by fine-tuned language models specialized in helpful and accurate responses.
    """)
        ...
        with st.spinner(f"Loading {model_name}... This may take a minute."):
            ...
            if peft_available:
                try:
                    ...
                    # If not LoRA, fallback to normal model
...
# Load selected model if different from previous
if st.session_state.current_model != model_choice:
    st.session_state.tokenizer, st.session_state.model = load_model(model_choice)
    ...
    if st.session_state.model_loaded:
        ...
        st.error(f"❌ Failed to load {model_choice}")
